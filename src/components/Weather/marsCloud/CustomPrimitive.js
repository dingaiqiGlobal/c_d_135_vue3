...
class CustomPrimitive {
  ...
  constructor(options) {
    ...
    const east_west_distance = Cesium.Cartesian3.distance(northeastCartesian, northwestCartesian)
    const north_south_distance = Cesium.Cartesian3.distance(northeastCartesian, southeastCartesian)
    ...
    // 创建模型矩阵
    const _local2world = Cesium.Transforms.eastNorthUpToFixedFrame(centerCartesian)
    const m = Cesium.Matrix4.fromScale(
      new Cesium.Cartesian3(east_west_distance, north_south_distance, av_height * 2),
      new Cesium.Matrix4(),
    )

    // 初始化包围球和模型矩阵
    this.boundingSphere = new Cesium.BoundingSphere(centerCartesian, maxDistance)
    this.modelMatrix = Cesium.Matrix4.multiply(_local2world, m, _local2world)
